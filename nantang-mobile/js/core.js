...
function avatarImg(seed,size){var s=size||40;return '<span style="display:inline-flex;position:relative;width:'+s+'px;height:'+s+'px;border-radius:50%;overflow:hidden;flex-shrink:0"><img src="'+avatarURL(seed,s)+'" width="'+s+'" height="'+s+'" style="border-radius:50%;object-fit:cover;width:100%;height:100%" alt="" onerror="this.style.display=\'none\'"><span class="ph-fallback" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:#e8ede6;pointer-events:none"><span class="ph-emoji" style="font-size:'+Math.round(s*.45)+'px">👤</span></span></span>'}
...
function showMy(opts){document.getElementById('villagePage').classList.add('hidden');document.getElementById('villagePage').classList.add('behind');document.getElementById('myPage').classList.remove('hidden');
  ...
  // M-1: 仅 file:// 开发态跑 NT 模拟器自检，HTTP 模式会计等式无意义（数据来自服务端）
