...
---
## 九、皇帝御批 v2(2026-07-31)· v0.3.1 终版

> 丞相重新思考"连续 3 周中断规则"——原推"换房中断"非最优,改"换房不中断、tenancy 关闭才中断"。其他 4 项按原推荐。

| # | 参数 | 御批(终) | 落地 |
|---|---|---|---|
| 1 | 治理权数据源 | **读 Tenancy**(真交了住宿费=真在地) | `routes/governance.py:check_*_right()` 读 User.tenancy 关联 |
| 2 | 退出提现约束 | **沿用 v3.1 §6.4**(7 天冷却 + trust≥60 + reserve 覆盖) | `routes/withdraw.py` 加 3 检查 |
| 3 | 连续 3 周中断规则 | **换房不中断、tenancy 关闭才中断**(真离开才中断) | `routes/governance.py:check_proposal_right()` 用 Tenancy.checkin_date 起算 |
| 4 | cron 硬检查失败 | **告警不阻断日结** | `_calc_escrow_drift()` 失败→`logger.critical()` + 邮件告警 |
| 5 | CV 公式 nt 范围 | **基础价(不含等级加成)** | `nt.py:_earn` 注释显式注明 `nt=基础价` |

**v0.3.1 关键修订(对照 v0.3)**:
- v0.3 治理权读 presence(可伪造 onclick) → v0.3.1 读 Tenancy(不可伪造)
- v0.3 "换房中断" → v0.3.1 "tenancy 关闭才中断"
- v0.3 退出无限额 → v0.3.1 v3.1 §6.4 三重约束
- v0.3 cron 硬检查没说失败行为 → v0.3.1 告警不阻断
