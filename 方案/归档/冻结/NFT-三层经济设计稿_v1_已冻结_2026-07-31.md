...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  裁定：本设计稿冻结，不推进 Phase 1-3。移入 方案/归档/冻结/。

  冻结理由（红队 B 审查 14 项发现 · 核心 1 条）：
    设计稿为解决 pool_refill 年通胀 7,300 NT，引入勋章消费机制，
    而勋章消费每月可产生 3,750+ NT 隐形补贴（年 45,000 NT）——
    **解决小漏洞，打开大 6 倍的口子。**

  丞相自纠（本稿 3 处失误）：
    ① 「池空溢出」是伪需求 — nt.py:1075-1076 池空时直接 400 拒绝，
       劳动不会 approve，labor_count 不会增加。溢出场景不存在。
    ② 「治理权重」直接违反砚仁 07-31 已定终审
       （原话：「CV/XP/等级/勋章，纯荣誉，全不挂治理权」）。丞相写进设计稿是失职。
    ③ 单向阀只是技术层面（不碰 nt_balance），经济层面不成立 —
       勋章能买本来要花 NT 的东西，价值照样在流动。

  保留的有效结论（继续执行）：
    · 删 pool_refill ✅ 砚仁 21:35 已批甲，NT-P0-6 卡在途
      （红队 B 确认：自平衡操作，删后会计等式仍成立，不影响住宿费/盈余划拨/自动调水）
    · NT 铁律：只从链上真钱充值来，平台绝不印

  解冻前置条件（如以后重启）：
    1. 砚仁先答「勋章能否全额消费平台商品」（红队 B Q1 · 总开关）
    2. 去掉治理权重（违反已定终审）
    3. 算清购买力出口上限 vs 印币入口的量级对比
    4. 幂等 key + 行锁（cron 重跑防重、badge stock 超卖）
