...
- 冰箱物品：v1.2 删除（二营实证前端读 localStorage，服务端填不进；改真机手动录入验证）
...
3. 填充后（无痕重登=服务端权威）：大厅 3 任务、时间线 3 条、在地人员 2 人、营地 2 个无 null、待校核 1 条。**v1.2 修订：删「冰箱 5 件」——二营复验实证冰箱 UI 读 localStorage 而非 InventoryItem 表，服务端 seed 填不进去（丞相开卡未核实读取源，设计失误认账）；冰箱验证改为真机手动录入 1 件（顺带测录入链路）**
4. 重复点填充不产生重复数据（幂等）
