...
| 卡 | 内容 | 施工 | 验收 | 状态 |
|---|---|---|---|---|
| D-19 | 测试闭环·后端（pytest 骨架 + D 系列回归） | 豆包 Codex | Claude Code | 🟡 已交付（704b57b）待验收 |
| D-20 | 测试闭环·契约链路冒烟（A-7 类闭环回归） | 豆包 Codex | Claude Code | 🟡 已交付（704b57b）待验收 |
| D-21 | 墓碑代码清理（先清单后动手） | Claude Code（跨端） | 豆包 Codex | 🟡 待施工（排在地基文档后） |
| D-22 | 部署总检脚本（依赖对账/?v=/环境变量/冒烟） | 豆包 Codex | Claude Code | 🟡 已交付（704b57b）待验收 |
| D-23 | 运维安全常态化（月扫/密钥清单/月度复核卡模板） | 豆包 Codex | Claude Code | 🟡 已交付（704b57b）待验收 |
| D-24 | index.html 版本号漏标补齐（D-22 实检发现 6 个 js 漏 ?v=） | Claude Code | 豆包 Codex | 🟡 待施工 |

## E 系列（架构线 · 2026-07-26 砚仁谕）

| 卡 | 内容 | 施工 | 验收 | 状态 |
|---|---|---|---|---|
| E-1 | 架构核验考古 + arch_check.py 机检（旧方案逐条证伪 + 真实架构图 + 差距清单） | 一营主察前端 / 二营主察服务端；脚本一营写 | 豆包 Codex | ✅ 验收过（待 push） |
